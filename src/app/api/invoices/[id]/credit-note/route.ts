...
export const POST = withAuthParams(async (request: NextRequest, { supabase, userId, companyId }, { id: invoiceId }) => {
    ...
    if (!invoice) {
        return ApiResponse.notFound('Faktura hittades inte')
    }

    // Don't allow credit notes on drafts
    if (invoice.status === 'Utkast') {
        return ApiResponse.badRequest('Kan inte kreditera en utkastfaktura — radera den istället')
    }
    ...
    if (!created) {
        return ApiResponse.serverError('Kunde inte skapa kreditfaktura')
    }
