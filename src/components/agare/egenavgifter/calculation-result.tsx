...
export function CalculationResult({ calculation }: CalculationResultProps) {
    return (
        <div className="space-y-6">
            <StatCardGrid>
                <StatCard
                    title="Totala egenavgifter"
                    value={formatCurrency(calculation.avgifter)}
                    icon={Percent}
                    description={`${(calculation.rate * 100).toFixed(2)}% av vinsten`}
                    trend="neutral"
                />
                <StatCard
                    title="Kvar efter avgifter"
                    value={formatCurrency(calculation.nettoEfterAvgifter)}
                    icon={Wallet}
                    description="Före inkomstskatt"
                    trend="up"
                />
                <StatCard
                    title="Månadslön (brutto)"
                    value={formatCurrency(calculation.monthlyNet)}
                    icon={Banknote}
                    description="Motsvarande bruttolön"
                />
            </StatCardGrid>
