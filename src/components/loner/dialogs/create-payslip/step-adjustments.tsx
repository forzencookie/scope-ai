...
export function StepAdjustments({
    ...
    return (
        <div className="bg-muted/30 rounded-lg p-3 flex flex-col h-[200px]">
            <ScrollArea className="flex-1 pr-4">
                <div className="space-y-3">
                    <div className="flex gap-2">
                        <div className="h-6 w-6 rounded-full bg-purple-100 dark:bg-purple-900/60 flex items-center justify-center flex-shrink-0">
                            <Bot className="h-3.5 w-3.5 text-purple-600" />
                        </div>
                        <div className="bg-white dark:bg-zinc-800 rounded-lg p-2.5 text-sm max-w-[85%] border shadow-sm">
                            <p>Något speciellt som påverkar {selectedEmp.name}s lön?</p>
                    ...
                    {chatMessages.map((msg, i) => (
                        <div key={i} className={cn("flex gap-2", msg.role === "user" ? "flex-row-reverse" : "")}>
                            <div className={cn(
                                ...
                                {msg.role === "ai" ? <Bot className="h-3.5 w-3.5 text-purple-600" /> : <User className="h-3.5 w-3.5 text-blue-600" />}
                            </div>
                            <div className={cn(
                                "rounded-lg p-2.5 text-sm max-w-[85%] border shadow-sm",
